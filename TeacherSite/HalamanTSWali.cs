...
namespace TeacherSite
{
    public partial class HalamanTSWali : Form
    {
        ...
        private void button_InputNilaiDataSiswaa_Click(object sender, EventArgs e)
        {
        ...
        public void DataWaliKelas()
        {
            OleDbConnection koneksi = new OleDbConnection(db);
            koneksi.Open();
            string query = "SELECT * FROM DATA_WALIKELAS WHERE NIP = '" + labelNIP.Text + "'";
            OleDbCommand cmd = new OleDbCommand();
            cmd.CommandText = query;
            cmd.Connection = koneksi;
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                label_KelasWaliKelas.Text = dt.Rows[i]["KELAS"].ToString();
            }

            string query_2 = "SELECT NIS, NAMA_LENGKAP FROM MASTER_SISWA WHERE KELAS = '" + label_KelasWaliKelas.Text + "' ORDER BY NAMA_LENGKAP";
            OleDbCommand cmd_2 = new OleDbCommand();
            cmd_2.CommandText = query_2;
            cmd_2.Connection = koneksi;
            ...
            DataTable dt_2 = new DataTable();
            da_2.Fill(dt_2);
            dataGridView_NamaSiswa.DataSource = dt_2;

            koneksi.Close();
        }


        private void HalamanTSWali_Load(object sender, EventArgs e)
        {
            DataJadwal();
            DataKelas();
            DataWaliKelas();
            ...
            for (int i = 0; i < dt_2.Rows.Count; i++)
            {
                ...
                label_TAWali.Text = dt_2.Rows[i]["TAHUN"].ToString();
                label_Smstrwali.Text = dt_2.Rows[i]["SEMESTER"].ToString();
            }

            koneksi.Close();
        }

        private void dataGridView_NamaSiswa_DoubleClick(object sender, EventArgs e)
        {
            OleDbConnection koneksi = new OleDbConnection(db);
            koneksi.Open();

            string IDCari = dataGridView_NamaSiswa.CurrentRow.Cells["NIS"].Value.ToString();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string NOTELP = dt.Rows[0]["NO_TLP"].ToString();
                string FOTO = dt.Rows[0]["FOTO_SISWA"].ToString();
                string ALAMAT = dt.Rows[0]["ALAMAT"].ToString();


                textBox_NamaSiswaWali.Text = NAMA_LENGKAP;
                textBox_NISSiswaWali.Text = NIS;
                textBox_TelpSiswaWali.Text = NOTELP;
                textBox_AlamatSiswaWali.Text = ALAMAT;

                pictureBox_DataSiswa.Image = Image.FromFile(FOTO);
                pictureBox_DataSiswa.SizeMode = PictureBoxSizeMode.StretchImage;
                pictureBox_DataSiswa.BorderStyle = BorderStyle.Fixed3D;


                //AMBIL DATA NILAI SISWA//
                string query_2 = "SELECT NAMA_PELAJARAN, NILAI, KETERANGAN FROM vw_DATA_NILAI WHERE NIS = '" + NIS + "' ORDER BY NAMA_PELAJARAN";
                OleDbCommand cmd_2 = new OleDbCommand();
                cmd_2.CommandText = query_2;
                cmd_2.Connection = koneksi;
                ...
                DataTable dt_2 = new DataTable();
                da_2.Fill(dt_2);
                dataGridView_DataNilaiKelasWali.DataSource = dt_2;
